...
class MLPWithSidecar(nn.Module):
    ...
    ``a = base(obs) + sidecar(sidecar_obs)``
    ...
    differ in width from the base input — e.g. an object-state observation group that the base
    ...
    def _build_sidecar(
        ...
        output_dim = self.base[-1].out_features
        self.sidecar = Sidecar(sidecar_input_dim, output_dim, sidecar_hidden_dims, sidecar_activation)

    def forward(self, obs: torch.Tensor, sidecar_obs: torch.Tensor | None = None) -> torch.Tensor:
        """Forward: ``base(obs) + sidecar(sidecar_obs)``.
        ...
        if sidecar_obs is None:
            sidecar_obs = obs
        return self.base(obs) + self.sidecar(sidecar_obs)
