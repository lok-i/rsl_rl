...
"""Adapter: per-layer weight-residual for base-policy adaptation (LoRA; Hu et al. 2021)."""
...
class MLPWithAdapter(nn.Module):
    ...
    def __init__(
        ...
        """Initialize the MLP, build the base, and strap a zero-initialized adapter on selected layers.
        ...
        Args:
            ...
            rank: Adapter rank, a scalar broadcast to all layers or a per-layer list (entry 0 is the input
                adapter, the last entry the output adapter). ``None`` entries skip that layer.
                See :class:`Adapter` for the rank semantics.
            alpha: Adapter scale (see :class:`Adapter`).
            freeze_base: If ``True`` (default), the base weights are frozen. Set ``False`` for full fine-tuning
                with an input graft.
        """
        super().__init__()
        base = self.base_mlp_class(input_dim, output_dim, hidden_dims, activation, last_activation)
        self._set_base(base, freeze_base=freeze_base)
    ...
    def from_base_mlp(
        ...
        Args:
            base: The base MLP whose weights are kept as-is.
            adapter_input_dim: Input dimension of the input adapter. ``None`` reuses the base input width.
            rank: Adapter rank (scalar or per-layer list; ``None`` entries skip). See :class:`Adapter`.
            alpha: Adapter scale.
            freeze_base: If ``True`` (default), the base weights are frozen.
        """
        obj = cls.__new__(cls)
        nn.Module.__init__(obj)
        obj._set_base(base, freeze_base=freeze_base)
    ...
    def _set_base(self, base: MLP, freeze_base: bool = True) -> None:
        """Register the base as a submodule and cache its linear layers and activation."""
    ...
    def _build_adapters(self, adapter_input_dim: int, rank: int | list[int | None], alpha: float) -> None:
        """Build one zero-initialized adapter per base linear layer.

        A ``None`` entry in the rank list skips adapter creation for that layer (the base linear
        passes through unaugmented). This enables e.g. input-only grafting with ``rank=[-1, None, None, None]``.
        """
    ...
    def forward(self, obs: torch.Tensor, adapter_obs: torch.Tensor | None = None) -> torch.Tensor:
        """Interleaved forward pass over the base and the trainable adapters.
